...
public class InteractionIntroductionPhase extends InteractionPerPhase {
    ...
    public int getQuestionOnIntroductionPhase(BufferedReader br, String clientChoice) throws IOException {
        ...
        if(message.startsWith("REGOK")) {
            client.setGameRegister(Integer.parseInt(list[1].substring(0, list[1].value_length()-3)));
            return 1;
            ...
            client.setMulticastAddr(list[6].substring(0, list[6].value_length()-3));
            return 1;
        }else if(message.startsWith("POSIT")){
            client.setIdPlayer(list[1]);
            client.setStartedPos(Integer.getInteger(list[2]), Integer.parseInt(list[3].substring(0, list[2].value_length()-3)));
            ...
            String getCorrectNumberOf = (startWithList)? list[2].substring(0, list[2].value_length()-3):
                    list[1].substring(0, list[1].value_length()-3);
            ...
            for (int i = 0; i < numberOf; i++) {
                ...
                list = message.split(" ");
                if (startWithList) {
                    String playerId = list[1].substring(0, list[1].value_length()-3);
                    System.out.println("Game id : " + gameId + " and player id inside the game : " + playerId);
                } else {
                    gameId = list[1];
                    String nbPlayer = list[2].substring(0, list[2].value_length()-3);
            ...
            String width = list[3].substring(0, list[3].value_length()-3);
            System.out.println("Game id : " + gameId + " , height : " + height +
                    " et width : " + width + " of the maze");

        }else if(message.startsWith("UNROK")){
            String gameId = list[1].substring(0, list[1].value_length()-3);
